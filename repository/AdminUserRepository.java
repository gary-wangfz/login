...
public interface AdminUserRepository extends JpaRepository<AdminUser, Integer>, QuerydslPredicateExecutor<AdminUser> {
    ...
    @Query(value = "select case when (select count(1) from  t_admin_user where login_phone=?1) >0 then 1 else 0 end as checkLoginPhone", nativeQuery = true)
    Integer checkLoginPhone(String loginPhone);
